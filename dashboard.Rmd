...
# need to figure out which variables to get 
# v17 %>% filter(geography=="county") %>% select(concept) %>% distinct()
names <-
  v17 %>% filter(
    geography == "county" &
      concept %in% c(
  ...
  ) %>% dplyr::select(c(name, concept, label)) %>% select(name) %>% unlist() %>% unname()
names_key <-
  v17 %>% filter(
    geography == "county" &
      concept %in% c(
  ...
  ) %>% dplyr::select(c(name, concept, label))
...
me <-  me %>% left_join(names_key, by = c("variable" = "name"))
me_means_transport <-
  me %>% filter(
...
# Interactive: Compare Commuting by County 
...
```{r}
checkboxGroupInput("counties", label = "Choose first county",
...
# Interactive: Compare Arrival Times and Length of Commute across Counties 
